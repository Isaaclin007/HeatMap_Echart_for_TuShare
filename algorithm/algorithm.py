...
def gain_pic_no_market(daily, category):
    ...
    # gain industry rate
    industry = daily[['industry', 'mktcap']].groupby(['industry'], as_index=False).agg(
        {"mktcap": sum})
    industry["rate"] = industry["mktcap"] / industry["mktcap"].sum()
    industry['industry_mktcap'] = industry['mktcap']
    industry['name'] = industry['industry']
    daily = pd.merge(daily, industry[['industry', 'industry_mktcap']], on=['industry'])
    daily['rate'] = daily['mktcap'] / daily['industry_mktcap']
    daily['industry'] = daily['industry']
    ...
    for i in range(len(industry)):
        inter_row = industry.iloc[i]
        inter_item = MapItem(inter_row, type_is='i')
        inter_daily_item = daily_item[(daily_item["industry"] == inter_row['industry'])]
    ...
    #     inter_row = industry.iloc[i]
    #     inter_item = MapItem(inter_row, type_is='m')
    #     inter_industry_item = industry_item[(industry_item["industry"] == inter_row['name'])]
    ...
    return json.dumps([total_item.return_json()])
    # return json.dumps([industry_item.return_json()])
...
if __name__ == '__main__':
    ...
    import pymo_interface as mogoIn
    ...
    daily, category = gain_data(db)
    print(daily.head(100))
    result = gain_pic_no_market(daily, category)
